...
- The processor is registered by the optional `detector_translation` plugin,
  not by `evio_parser`, and inserts DigiHits before downstream processors
  consume the physics event.
...
- Each setup plugin owns its route registrations. The registry freezes on the
  first translated event, after all JANA service initialization has completed.
